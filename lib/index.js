...
{"name": "스피츠", "mbti": "1010", "header": "느긋하고 독립적인", "text": "<div class='result__text--bold'>스피츠</div>혼자 가만히 누워서 사색에 잠기는 당신! 주인에게만은 사랑꾼인 모습이 아름답군요.", "list": ["래브라도 리트리버", "웰시코기"], "specialHeader": "우리집 귀요미 ", "specialText": "<div class='result__text--bold'>뚜띠</div>제가 사랑하는 우리집 강아지 뚜띠입니다. 견종은 스피츠이고 저를 닮아 겁돌이에 집돌이이지만, 낯선사람 앞에서는 누구보다 용감하고 간식앞에서는 누구보다 눈이 초롱초롱 빛나는 사랑스러운 뚜띠입니다."},
...
{"name": "비숑 프리제", "mbti": "0001", "header": "선하고 정이 많은", "text": "<div class='result__text--bold'>비숑<br>프리제</div>주인의 기분대로 뭐든지 맞춰주는 당신! 당신을 반려견으로 둔 주인은 정말 복받은 사람이네요.", "list": ["시바", "버니즈 마운틴 독"]},
...
async function init() {
  ...
  dog = mbtiCompare[mbtiPercent.indexOf(Math.max(...mbtiPercent))];
  setResult();
}
...
function setResult(){
  ...
  if(dog.name == "스피츠"){
    if(Math.random() > .8){
      $(".result__image>img")[0].src = `./img/dog/뚜띠.png`;
      $(".result__text")[0].innerHTML = dog.specialText;
      $(".result__text--header")[0].innerHTML = dog.specialHeader;
    }
  }
